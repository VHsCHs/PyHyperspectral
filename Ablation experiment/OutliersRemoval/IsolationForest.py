...
#     forest = IsolationForest(n_estimators=100, max_samples='auto', contamination=i, max_features=1.0, bootstrap=False,
#                              n_jobs=1, random_state=1, verbose=0,behaviour='new')
#     forest.fit(np.hstack([X_array, Y_array.reshape(-1, 1)]))
#     index = forest.predict(np.hstack([X_array, Y_array.reshape(-1, 1)]))
#
...
#
# plt.savefig('D:\Desktop\hyper\Isolation contamination.jpeg', dpi=400)
# plt.show()

forest = IsolationForest(n_estimators=100, max_samples='auto', contamination=0.05, max_features=1.0, bootstrap=False,
                         n_jobs=1, random_state=1, verbose=0,behaviour='new')
forest.fit(np.hstack([X_array, Y_array.reshape(-1, 1)]))
index = forest.predict(np.hstack([X_array, Y_array.reshape(-1, 1)]))
...
Y_valid, Y_invalid = error_wipe(Y_array, index)
print(all_np(index))
plt.figure('IsolationForest',figsize=(4.8,5.4),dpi=100)
...
plt.savefig('D:\Desktop\hyper\IsolationForest.jpg',dpi=100)
plt.show()