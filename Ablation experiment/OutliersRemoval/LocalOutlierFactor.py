...
# 离群值检测
# '''
# plt.figure('LocalOutlierFactor contamination', figsize=(9.6, 3.8), dpi=200)
# # plt.suptitle('LocalOutlierFactor contamination', fontsize=20)
# for num, i in enumerate(list([0.01, 0.03, 0.05, 0.1, 0.2, 0.5])):
#     LOF = LocalOutlierFactor(n_neighbors=20, algorithm='auto', leaf_size=30, metric='minkowski', p=2, metric_params=None, contamination=i, n_jobs=1,novelty=True)
#     LOF.fit(np.hstack([X_array, Y_array.reshape(-1, 1)]))
#     index = LOF.predict(np.hstack([X_array, Y_array.reshape(-1, 1)]))
#
...
#
# plt.savefig('D:\Desktop\hyper\LOF contamination.jpeg', dpi=400)
# plt.show()


LOF = LocalOutlierFactor(n_neighbors=20, algorithm='auto', leaf_size=30, metric='minkowski', p=2, metric_params=None, contamination=0.1, n_jobs=1,novelty=True)
LOF.fit(np.hstack([X_array, Y_array.reshape(-1, 1)]))
index = LOF.predict(np.hstack([X_array, Y_array.reshape(-1, 1)]))
print(all_np(index))
X_valid, X_invalid = error_wipe(X_array, index)
...
plt.figure('LocalOutlierFactor',figsize=(4.8,5.4),dpi=100)
plt.title('LocalOutlierFactor',fontsize=20)
plt.scatter(X_array[:,48],Y_array,color='teal', s=10)
plt.scatter(X_valid[:,48],Y_valid,color='chocolate', s=15)
plt.savefig('D:\Desktop\hyper\LocalOutlierFactor.jpg',dpi=100)
plt.show()