...
'''
ACTION:
    create csv-file to save circles imformation
JOBS:
    judge whether SaveCircles folder is Exist
    create SaveCircles folder
    create csv-file and write header
RETURN:
    SaveCircles:csv-raw-file
    circles_csv:csv-file open with csv-lib
'''
if os.path.exists(os.path.abspath(os.path.join(path, '..')) + '\\SaveParameter\\') == False:
    os.mkdir(os.path.abspath(os.path.join(path, '..')) + '\\SaveParameter\\')
SaveCircles = open(
    os.path.abspath(os.path.join(path, '..')) + '\\SaveParameter\\' + Project_Time + '-SaveParameter.csv',
    'w', newline='')
circles_csv = csv.writer(SaveCircles)
csv_header = ['filename', 'X-position', 'Y-position', 'Radius', 'B', 'K', 'Xmin', 'Ymin', 'Xmax', 'Ymax']
circles_csv.writerow(csv_header)

'''
line number and camera parameter
'''
plt_line = 15
plt_row = math.ceil(np.shape(csv_file)[0] * 1 / plt_line)
...
for item in csv_file:
    ...
    with open(path + '/' + item) as f:
        ...
        cannyimg = color_img.astype(np.uint8)
        ...
        '''cv2 threshold-->>gain contrast'''
        '''Optional:cv2.adaptiveThreshold-->>have no good result'''
        ret, cannyimg = cv2.threshold(cannyimg, 40, 255, cv2.THRESH_BINARY)
        ...
        xmin = (Linear_X(bot_point, B, K)) + L_offset
        ymin = bot_point
        xmax = (Linear_X(top_point, B, K)) + R_offset
        ymax = top_point
        ...
        try:
            ...
            '''
            get item name & wirte circles&lines parameter
            '''
            while True:
                ...
                if item[1] != '':
                    ...
                    break
            print(item[0])
            csv_line = [item[0], circle[0], circle[1], circle[2], B, K, xmin, ymin, xmax, ymax]
            print(csv_line)
            circles_csv.writerow(csv_line)
            ...
            plt.title(item[0] + ' Canny', verticalalignment='baseline',
                      horizontalalignment='center', fontsize=10)
            plt.xticks([]), plt.yticks([])
            num = num + 1
            ...
            # plt.title(item[0] + ' Gray', verticalalignment='baseline',
            #           horizontalalignment='center', fontsize=10)
            # plt.xticks([]), plt.yticks([])
            # num = num + 1
...
mail = MAIL(username='786605549@qq.com', passwd='acrjzgvyagpjbegj', mailserver='smtp.qq.com', port=465)
mail.send(receive='786605549@qq.com', subtitle=os.path.basename(sys.argv[0]).split(".")[0],
          body='Total Time = ' + str(math.ceil(tol_time)) + 's' + '\nDone!')

'''
plt save picture
'''
if os.path.exists(os.path.abspath(os.path.join(path, '..')) + '\\SaveFig\\') == False:
    os.mkdir(os.path.abspath(os.path.join(path, '..')) + '\\SaveFIg\\')
plt.savefig(os.path.abspath(os.path.join(path, '..')) + '\\SaveFig\\' + Project_Time + '-SaveFig.jpg', dpi=400)
